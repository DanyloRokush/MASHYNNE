from ownnumpy import matrixMultMatrix, matrixMultVector, vectorMultVector
import numpy as np
import time
import random
...
print(f"own {trackTimeForFunc(lambda: matrixMultMatrix(A, B))}")
...
print(f"own {trackTimeForFunc(lambda: matrixMultVector(A, B))}")
...
print(f"\nVector({n}) X Vector({n})")
print(f"own {trackTimeForFunc(lambda: vectorMultVector(A, B))}")
